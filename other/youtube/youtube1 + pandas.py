...
def run():
    try:
        with SandboxClient(TOKEN) as client:
            Hola(client).report()
            # dev1 = Hola(client).report()
            # dev1.ca
            # Hola(client).cast_money()
...
class Hola:
    ...
    def report(self):
        """
        Получаю все операции по одному выбранному счету
        :return:
        """
        dataframes = []
